...
🟡 T - 목표 (Task)
모임 종료 후 사용자 간 별점 평가를 통해 상호 신뢰도를 반영하고, 누적 점수 기반으로 평균 평가 점수를 계산하여 실버~마스터 등급을 자동 부여하는 시스템을 구현하는 것이 목표였습니다.
...
🟡 R - 결과 (Result)
등급을 통해 사용자의 모임 활동 신뢰도가 시각화되었고, 모임 신청자가 모임에 대한 참여 의사결정을 할떄 합리적인 선택을 할 수 있도록 도움을 주었습니다.

사용자 참여 유도 효과를 보았으며, “믿고 만날 수 있는 사람을 찾기 쉬워졌다”는 긍정적인 피드백을 수렴하였고

데이터 처리와 사용자 경험을 모두 고려한 기능 구현 능력을 키울 수 있었습니다.
